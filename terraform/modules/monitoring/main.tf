...
resource "aws_cloudwatch_metric_alarm" "node_cpu" {
  for_each = var.enable_alarms && var.enable_node_alarms ? toset(var.autoscaling_group_names) : {}
...
resource "aws_cloudwatch_metric_alarm" "node_memory" {
  for_each = var.enable_alarms && var.enable_node_alarms && var.enable_memory_metrics ? toset(var.autoscaling_group_names) : {}
